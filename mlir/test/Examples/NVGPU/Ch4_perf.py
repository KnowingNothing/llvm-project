...
#   d -> memref<MxNxf32>
@NVDSL.mlir_func
def gemm_multistage(a_dev, b_dev, d_dev, num_stages):
    ...
    a_tma = TMA([128, 64], a_dev.type, swizzle=sw)
    b_tma = TMA([64, 64], b_dev.type, swizzle=sw)
    ...
    size_a = get_type_size(a_dev.type.element_type) * TILE_M * TILE_K
    size_b = get_type_size(b_dev.type.element_type) * TILE_N * TILE_K
...
a_torch = torch.tensor(a, dtype=torch.float16).cuda()
b_torch = torch.tensor(b, dtype=torch.float16).cuda()
d_torch = torch.tensor(d, dtype=torch.float32).cuda()

run_func = gemm_multistage(a_torch, b_torch, d_torch, num_stages=7)
run_func()


# # Verify MLIR with reference computation
# ref_d = a.astype(np.float16) @ b.astype(np.float16)
# np.testing.assert_allclose(d_torch.cpu().numpy(), ref_d, rtol=5e-03, atol=1e-01)
...
_, time_cost = perf_func(run_func, 1000, 20)
print(f"Time cost: {time_cost}, TFLOPS={M*N*K*2/time_cost*1000/1e12}")
